...
- QA frontend must show the newest message after entering or switching a session and after local submission. Polling may follow only while the user is near the bottom; when the user reads older history, preserve their position and expose an explicit return-to-latest control.
...
- Knowledge document lists must use the backend `pageNo`, `pageSize`, `keyword`, and `indexStatus` contract, preserve the active page during polling, reset to page one after base/filter changes or upload, and move back after deleting the only row on a later page.
...
- Compliance review display numbers may repeat in source templates, but persisted internal rule IDs must remain unique and stable by parse order (for example, `RULE-004`, `RULE-004-2`). Preserve the original display number separately, validate uniqueness before model calls and database writes, and replace a record's rule results atomically without weakening the database unique constraint.
...
- Policy article `publishDate` remains the source publication date; show `createdAt` as first ingestion and label `updatedAt` as record update time because indexing state changes also update it. Single-article source URLs may be saved but must warn that they cannot discover later articles, and dated URL candidates must be prioritized stably before the crawler article limit is applied.

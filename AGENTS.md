...
- Review submission must create a `COMPLIANCE_REVIEW` `QUEUED` task and durable `task_outbox` event, then return the `PENDING` review record immediately. The Worker must revalidate the project through system-safe project/template/file access methods and call Python Agent through `invokeAgentForSystem`; it must not use request-thread `SecurityContext`. Frontend review pages must poll non-terminal records instead of holding the submit request open for model completion.
- Review submission supports exactly one primary review file plus zero or more PDF/DOC/DOCX reference files and project knowledge-base IDs. Reference file/knowledge-base IDs are snapshotted on the record, revalidated by the Worker, and model citations must be filtered against the validated source whitelist before persistence and display.

...
- 报告：列表、详情、重新生成、下载 Word/PDF；新建报告只加载已启用的 DOCX 报告模板，仅已完成报告允许下载，生成中报告禁止重复发起生成。
...
- 文件解析：仅 `SUCCESS` 解析记录允许查看内容，仅 `FAILED` 解析记录允许重试解析；可用于报告生成的模板上传限制为 DOCX，避免上传后成功扫描变量、生成时才因格式不支持而失败。

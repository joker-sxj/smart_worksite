...
本阶段已完成 OCR 生产闭环的五个功能点，并完成 55 个可重复验证场景：55 通过，0 失败。测试使用本地模型链路，未启用云端回退。Java、Python、前端构建和全量自动化测试均通过。
...
- 记录 33：通过 Chrome 可见上传按钮选择真实测试文件并提交，自定义合同识别成功，3 个字段全部准确展示。
- Chrome OCR 页面已展示记录 28-33；切换到“自定义字段识别”后已显示结构化字段编辑器；该页面本轮控制台无 warn/error。

## 55 场景结果

执行脚本：服务器 `/tmp/stage6-ocr-acceptance.sh`。接口与环境矩阵结果：`PASS=54 FAIL=0`；另完成 1 个 Chrome 端到端上传、提交、轮询和结果展示场景，合计 `PASS=55 FAIL=0`。
...
- Chrome 直接上传已补测通过。自动化必须点击可见上传按钮来触发文件选择器；直接点击隐藏的 `input[type=file]` 会被浏览器扩展拦截，这属于自动化方式限制，不是业务上传故障。

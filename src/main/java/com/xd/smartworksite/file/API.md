...
Excel 日期单元格按完整日期进入解析文本和知识索引，并在结构化数据的 `cells` 中保留单元格地址、原始数值、标准日期、原表显示值和格式编号。旧版 `.xls` 在格式字符串缺失或仅有 POI `reserved-*` 占位时，受控识别区域日期格式编号 `27-36`、`50-58`，不会再以日期序列值输出；公式日期使用工作簿缓存结果生成原表显示值。没有可靠日期样式的普通数字不会被推断为日期或年份。
